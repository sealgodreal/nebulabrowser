...
class UVServiceWorker extends EventEmitter {
    ...
    async fetch({ request }) {
        ...
        try {
            ...
            if (response.status === 500) {
                return Promise.reject(new Error('Bare server HTTP 500 for ' + ultraviolet.meta.url.href));
            };
    ...
    getBarerResponse(response) {
        ...
        if (!rawHeader || statusHeader === null) {
            throw new Error(
                'Bare server returned HTTP ' + response.status + ' without x-bare-* headers. ' +
                'The bare endpoint (/seal/v1/) is unreachable or a tunnel/proxy served a warning page instead.'
            );
        }
        ...
        try {
            ...
            throw new Error('Bare server sent unreadable x-bare-headers.');
        }
        ...
        if (!Number.isFinite(status) || status < 200 || status > 599) {
            throw new Error('Bare server returned invalid status ' + statusHeader + '.');
        };
...
class RequestContext {
    ...
    get send() {
        ...
        try {
            if (typeof userKey !== 'undefined' && userKey) headers['userKey'] = userKey;
        } catch (e) {};
